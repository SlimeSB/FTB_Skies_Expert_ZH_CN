...
global.postChangeDimension = event => {
	try {
		console.log(
			`Entity: ${event.entity.getName().string} changed dimension to: ${event
				.getTo()
				.location()
				.toString()}`
		)
		...
		if (
			!isOrbit(toDimension) &&
			!isPlanet(toDimension) &&
			!toDimension.includes('ftbteamdimensions')
		)
			return
		...
		Utils.server.scheduleInTicks(20, () => {
			let entities = player.level
				.getEntities()
				.filter(entity => entity.getType() == 'ad_astra:lander')
			entities.forEach(lander => {
				...
				Utils.server.runCommandSilent(
					`execute in ${toDimension} run forceload remove 0 0`
				)
			})
...
function spawnSpaceStation(player, dimension) {
	...
	let blockPos = new BlockPos(
		player.x + structureOffset.x,
		structureOffset.y,
		player.z + structureOffset.z
	)
